 ...
 * Goes through the direct zones and indirect zones.
 * Sadly, does not check the double indirect zones.
 */
void get_target(FILE *image, args *args, inode *inodes) {
   ...
   for (i = 0; i < DIRECT_ZONES; i++) {
      size_left = copy_out_zone(image, dst,
   ...
   int j;
   uint32_t double_indir_zones[zoneSize / sizeof(uint32_t)];
   if (size_left > 0) {
      //There are still file contents in double indirect zones.
      fseek(image, (zoneSize * target->two_indirect) + part_offset,
            SEEK_SET);
      fread(double_indir_zones, zoneSize, 1, image);
      for (i = 0; i < num_zones; i ++) {
         fseek(image, (zoneSize * double_indir_zones[i]) + part_offset,
               SEEK_SET);
         fread(indir_zones, zoneSize, 1, image);
         for (j = 0; j < num_zones; j++) {
            size_left = copy_out_zone(image, dst, 
                  size_left, indir_zones[j]);
            if (size_left <= 0) 
               break;
         }
         if (size_left <= 0) 
...
/*
 * copy_out_zone
 * Copies the content of the given zone number out to
 * the destination FILE pointer. If there is no given
 * destination, then the output is to stdout.
 */
int copy_out_zone(FILE *image, FILE *dst, 
   ...
   res = fseek(image, (zoneSize * zone_num) + part_offset,
         SEEK_SET);
   if(res == -1) {
      perror("Error while seeking");
      exit(1);
